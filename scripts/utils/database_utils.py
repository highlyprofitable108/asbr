...
def read_sql_file(file_name):
    ...
    """
    # File path is determined by adding the file name to a predefined SQL scripts directory path
...
def csv_to_dict_list(csv_string):
    ...
    """
    # csv.DictReader takes a file-like object, here we use StringIO to transform string to file-like object
...
def backup_db(db_path_with_name: str) -> None:
    ...
    Args:
        db_path_with_name (str): The path to the database file along with the database filename.
    """
    ...
    try:
        # The 'source_db' is opened in read-only mode, and its content is copied to 'backup_db'
        with sqlite3.connect(f'file:{db_path_with_name}?mode=ro', uri=True) as source_db:
    ...
    try:
        # The backup file is opened in read-binary mode and its content is copied to a gzip file
...
def get_historical_raw_data_event_ids(cursor: sqlite3.Cursor) -> None:
    ...
    data_to_insert = [
        (
            row['calendar_year'], row['date'], row['event_id'], row['event_name'], 
            row['sg_categories'], row['event_name']
        ) for row in data
    ]
...
def get_round_scoring(cursor: sqlite3.Cursor) -> None:
    ...
    for tour in constants['tours']:
        for year in constants['years']:
            ...
            data_to_insert = [
                (
                    row.get('tour'), row.get('year'), row.get('date'), row.get('event_id'), row.get('event_name'), row.get('dg_id'),
                    row.get('player_name'), row.get('fin_text'), row.get('round_num'), row.get('course_num'),
                    row.get('course_name'), row.get('course_par'), row.get('round_score'), row.get('sg_putt'), row.get('sg_arg'),
                    row.get('sg_app'), row.get('sg_ott'), row.get('sg_t2g'), row.get('sg_total'), row.get('driving_dist'),
                    row.get('driving_acc'), row.get('gir'), row.get('scrambling'), row.get('prox_rgh'), row.get('prox_fw')
                ) for row in data
            ]
...
def get_player_stats(db_conn: sqlite3.Connection) -> None:
    """
    Retrieves raw player stats, calculates averages and updates the players table in the database.

    Args:
        db_conn (sqlite3.Connection): A SQLite connection to the database.
    """
    # Build the API endpoint
    endpoint = f"{constants['base_url']}/preds/skill-ratings?display=value&file_format={constants['file_format']}&key={constants['api_key']}"
    ...
    try:
        ...
        data_to_insert = [
            (
                row['dg_id'], row['player_name'], row['sg_putt'], row['sg_arg'], row['sg_app'], row['sg_ott'], 
                float(row['sg_arg']) + float(row['sg_app']) + float(row['sg_ott']), row['sg_total'], row['dg_id']
            ) for row in data
        ]
        cursor.executemany(sql_script, data_to_insert)
        db_conn.commit()
...
def populate_course_data(db_conn: sqlite3.Connection) -> None:
    """
    Retrieves and updates course data, and fetches location data for courses if necessary.
    ...
    try:
        # Insert new courses into the courses table or ignore if they already exist
...
def populate_weather_table(db_conn: sqlite3.Connection) -> None:
    """
    Retrieves weather data for each unique date and course combination and updates the weather table accordingly.
    ...
    try:
        ...
        for date_course in unique_date_course_combinations:
            ...
            cursor.execute("SELECT latitude, longitude FROM courses WHERE course_name = ?", (course_name,))
            ...
            # Check if weather data for the given date and course already exists
            cursor.execute("SELECT * FROM weather WHERE date = ? AND course_name = ?", (date, course_name))
            ...
            if existing_weather_data is None:
                ...
                if weather_data is not None:
                    cursor.execute("""
                        INSERT INTO weather (date, course_name, temperature, wind_speed, humidity, precipitation)
                        VALUES (?, ?, ?, ?, ?, ?)
