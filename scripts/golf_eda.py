...
def load_and_process_data():
    ...
    """
    conditions = [f"{col} BETWEEN -1000 AND 1000" for col in features]
    where_clause = " AND ".join(conditions)
    where_clause += " AND " + " AND ".join([f"{col} IS NOT NULL" for col in features])

    with sqlite3.connect(constants['database_path']) as db_conn:
        df = pd.read_sql_query(f"SELECT * FROM model_data WHERE {where_clause}", db_conn)
...
def print_correlations(correlations, n):
    print(f"Top {n} correlations with 'sg_total':")
...
def main():
    ...
    correlations = numeric_df.corr()[constants['target_variable']].drop(constants['target_variable'])
    sorted_correlations = correlations.abs().sort_values(ascending=False)
    
    print_correlations(sorted_correlations, 20)
